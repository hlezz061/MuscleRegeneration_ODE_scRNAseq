...
mckellar <- readRDS(file.path(annot_dir, "mckellar_harmony_umap.rds"))
...
# Few Sanity chekcks
mckellar$final_annotation <- factor(
  final_annotations[as.numeric(as.character(mckellar$seurat_clusters)) + 1],
  levels = unique(na.omit(final_annotations))
)

table(mckellar$final_annotation, useNA = "always")
...
p_annot <- DimPlot(
  ...
  label.size = 4,
  repel = TRUE
) + ggtitle("McKellar UMAP – Final Annotation")
...
# =======================
annotated_path <- file.path(annot_dir, "mckellar_annotated_final.rds")
saveRDS(mckellar, annotated_path)
...
proportions_by_day <- calculate_proportions(mckellar, "injury.days")
proportions_by_sample <- calculate_proportions(mckellar, "sample")
...
head(mckellar@meta.data)

# What assays are active 
Assays(mckellar)
DefaultAssay(mckellar)

# How many cells here? 
table(mckellar$seurat_clusters %in% c("13", "19"))

##############################################################################
# Script: V4_subcluster_muscs.R : SAME RESOLUTION AS BEFORE
# Purpose: Subcluster MuSCs (clusters 13, 19 ONLY) from McKellar dataset (V4)
##############################################################################
...
mckellar <- readRDS(file.path(annotation_dir, "mckellar_annotated_final.rds"))
DefaultAssay(mckellar) <- "RNA"
...
# ====================================
cat(blue$bold("\n Subsetting MuSC clusters (13, 19 ONLY)...\n"))
musc_obj <- subset(mckellar, idents = c("13", "19"))
...
# Use the same global integration — reuse global PCA or UMAP embedding
# We'll recluster directly using the same dimensions

cat(blue$bold("\n Finding subclusters directly (no re-Harmony)...\n"))
musc_obj <- FindNeighbors(musc_obj, dims = 1:20)
musc_obj <- FindClusters(musc_obj, resolution = 0.3)  # change this to test higher resolutions

# For new UMAP, reuse the existing embedding if you want — or run a simple UMAP:
...
#     Save Object
# =======================
saveRDS(musc_obj, file = file.path(subcluster_dir, "mckellar_MuSCs_subclustered.rds"))
cat(green$bold("\n MuSC subclustering complete and saved.\n"))
cat(cyan(" → UMAP saved to: "), file.path(subcluster_dir, "MuSCs_UMAP.png"), "\n")
cat(cyan(" → Object saved to: "), file.path(subcluster_dir, "mckellar_MuSCs_subclustered.rds"), "\n")
...
marker_output     <- check_marker_coverage(Renad_markers_titlecase, musc_obj, "McKellar MuSCs – RNA assay")
...
muscs_final_annotations <- c(
  ...
  "QSCs",                     # Cluster 1
  "ASCs",                     # Cluster 2
  "Differentiating Myocytes", # Cluster 3
  "ASCs"                      # Cluster 4
)
...
p_musc_annot <- DimPlot(
  ...
  label.size = 4,
  repel = TRUE
) + ggtitle("McKellar MuSC Subclusters – Final Annotation")
...
cat(green$bold("\n Final annotated MuSC UMAP saved to:\n"))
cat(green(file.path(subcluster_dir, "MuSCs_UMAP_final_annotation.png")), "\n")

# ===========================
#  Day-wise Annotated UMAPs
# ===========================

day_umap_dir <- file.path(subcluster_dir, "Day_UMAPs")
dir.create(day_umap_dir, recursive = TRUE, showWarnings = FALSE)
...
#  Save Annotated Object
# =======================
annotated_musc_path <- file.path(subcluster_dir, "mckellar_MuSCs_annotated_final.rds")
...
# Save to Excel
library(writexl)
musc_excel_path <- file.path(subcluster_dir, "UMAP_Coordinates_MuSCs.xlsx")
write_xlsx(umap_musc, musc_excel_path)

# Confirmation message
cat(green$bold("MuSC UMAP coordinate table saved to:\n"))
cat(green(musc_excel_path), "\n")

######################################### Subcluster Proportions + Plots ########################################

# ==========================
#  Barplot Output Directory
# =========================
...
# ================================
#     MuSC Proportion Plotting
# ================================
generate_bar_plot <- function(data, group_by, output_file, title, annotation_col = "muscs_annotation") {
...
generate_line_plot <- function(data, output_file, title, annotation_col = "muscs_annotation") {
...
generate_horizontal_bar_plot <- function(data, group_by, output_file, title, annotation_col = "muscs_annotation") {
...
generate_bar_plot(
  musc_prop_day, "injury.days",
  file.path(barplot_dir, "MuSCs_BarPlot_by_injury_days.png"),
  "MuSC Subcluster Proportions by Injury Day"
)

generate_bar_plot(
  musc_prop_sample, "sample",
  file.path(barplot_dir, "MuSCs_BarPlot_by_sample.png"),
  "MuSC Subcluster Proportions by Sample"
)

generate_line_plot(
  musc_prop_day,
  file.path(barplot_dir, "MuSCs_LinePlot_by_injury_days.png"),
  "MuSC Subcluster Proportions Over Time"
)

# Individual barplots by cell type
for (label in unique(musc_prop_day$muscs_annotation)) {
  ...
  generate_bar_plot(
    subset_data, "injury.days", out_file,
    paste(label, "- Proportion by Injury Day")
  )
}

generate_horizontal_bar_plot(
  musc_prop_day, "injury.days",
  file.path(barplot_dir, "MuSCs_Horizontal_BarPlot_by_injury_days.png"),
  "MuSC Proportions (Horizontal by Injury Day)"
)

generate_horizontal_bar_plot(
  musc_prop_sample, "sample",
  file.path(barplot_dir, "MuSCs_Horizontal_BarPlot_by_sample.png"),
  "MuSC Proportions (Horizontal by Sample)"
)

cat(magenta$bold("\n MuSC annotation and proportion plots complete.\n"))
