# =====================================================
# Load and inspect the extracted McKellar data  
# Inspect and decide on the annotation 
# Also extract proportions for the annotations 
# =====================================================

# ─────────────────────────────────────────────────────────────────────────────
# Setup for McKellar dataset (fresh start)
# ─────────────────────────────────────────────────────────────────────────────
...
mckellar_rds <- file.path(seurat_dir, "ref_McKellar.rds")

# == Output folders specific to this version of McKellar
mckellar_dir              <- base_dir  # Keep for consistency
...
# == Load object
cat(blue$bold("\n Loading McKellar Seurat object...\n"))
mckellar <- readRDS(mckellar_rds)
DefaultAssay(mckellar) <- "RNA"
...
mckellar <- ScaleData(mckellar, verbose = TRUE)
mckellar <- RunPCA(mckellar, npcs = 20, verbose = TRUE)
...
final_umap_rds <- file.path(annotation_base_dir, "Final UMAP Annotation", "mckellar_harmony_umap.rds")
dir.create(dirname(final_umap_rds), recursive = TRUE, showWarnings = FALSE)
saveRDS(mckellar, final_umap_rds)
...
log_file <- file.path(log_dir, paste0("McKellar_log_", format(Sys.Date(), "%Y%m%d"), ".txt"))
log_con  <- file(log_file, open = "wt")

writeLines("========= McKellar Dataset Diagnostics =========\n", con = log_con)

# == Basic dimensions
writeLines(paste0(" Cells: ", ncol(mckellar)), con = log_con)
writeLines(paste0(" Genes: ", nrow(mckellar)), con = log_con)

# == Assay info
writeLines(paste0("\n Default assay: ", DefaultAssay(mckellar)), con = log_con)
writeLines(paste0("Available assays: ", paste(Assays(mckellar), collapse = ", ")), con = log_con)

# == Reductions present
writeLines(paste0("\n Reductions: ", paste(Reductions(mckellar), collapse = ", ")), con = log_con)

# == Metadata columns
writeLines("\n Metadata columns:\n", con = log_con)
writeLines(paste(colnames(mckellar@meta.data), collapse = "\n"), con = log_con)

# == Cluster info
if ("seurat_clusters" %in% colnames(mckellar@meta.data)) {
  cluster_counts <- table(mckellar$seurat_clusters)
  writeLines("\n Cluster levels:\n", con = log_con)
  writeLines(paste0(levels(factor(mckellar$seurat_clusters)), collapse = ", "), con = log_con)
  writeLines("\n Cells per cluster:\n", con = log_con)
  writeLines(capture.output(cluster_counts), con = log_con)
}

# == Annotation info
if ("harmony_factorIDs" %in% colnames(mckellar@meta.data)) {
  annotation_counts <- table(mckellar$harmony_factorIDs)
  writeLines("\n️ Annotation labels (harmony_factorIDs):\n", con = log_con)
  writeLines(paste0(levels(factor(mckellar$harmony_factorIDs)), collapse = ", "), con = log_con)
  writeLines("\n Cells per annotation:\n", con = log_con)
  writeLines(capture.output(annotation_counts), con = log_con)
}

# == Timepoint + sample info
if ("injury.days" %in% colnames(mckellar@meta.data)) {
  day_counts <- table(mckellar$injury.days)
  writeLines("\n Timepoints (injury.days):\n", con = log_con)
  writeLines(paste0(levels(factor(mckellar$injury.days)), collapse = ", "), con = log_con)
  writeLines("\n Cells per timepoint:\n", con = log_con)
  writeLines(capture.output(day_counts), con = log_con)
}

if ("sample" %in% colnames(mckellar@meta.data)) {
  sample_counts <- table(mckellar$sample)
  writeLines("\n Samples:\n", con = log_con)
  writeLines(paste0(levels(factor(mckellar$sample)), collapse = ", "), con = log_con)
  writeLines("\n Cells per sample:\n", con = log_con)
  writeLines(capture.output(sample_counts), con = log_con)
}

# == Top expressed genes
top_genes <- head(sort(Matrix::rowSums(GetAssayData(mckellar, slot = "counts")), decreasing = TRUE), 20)
writeLines("\n Top 20 most expressed genes:\n", con = log_con)
writeLines(capture.output(top_genes), con = log_con)

# == Preview UMAP embedding
if ("umap_fresh" %in% Reductions(mckellar)) {
  writeLines("\n UMAP (umap_fresh) coordinate preview:\n", con = log_con)
  writeLines(capture.output(head(Embeddings(mckellar, "umap_fresh"))), con = log_con)
}
...
Idents(mckellar) <- "seurat_clusters"
...
p_cluster <- DimPlot(
  ...
  ggtitle("McKellar – Harmony-Corrected UMAP by Clusters") +
  theme_minimal() +
  theme(
...
# == Print and save
print(p_cluster)
ggsave(file.path(umap_dir, "McKellar_UMAP_clusters_only_Harmony.png"), plot = p_cluster, width = 9, height = 7, dpi = 300)
...
mckellar$harmony_factorIDs <- factor(mckellar$harmony_factorIDs,
                                     levels = sort(unique(mckellar$harmony_factorIDs)))
...
p_annot <- DimPlot(
  ...
  ggtitle("McKellar – Harmony-Corrected UMAP by Cosgrove Annotation") +  
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
    legend.title = element_blank(),
...
p_sample <- DimPlot(
  ...
  ggtitle("McKellar – Harmony-Corrected UMAP by Sample") +  
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
    legend.title = element_blank(),
...
# == Save updated UMAPs
ggsave(file.path(umap_dir, "McKellar_UMAP_annotation_Harmony.png"), plot = p_annot, width = 9, height = 7, dpi = 300)
ggsave(file.path(umap_dir, "McKellar_UMAP_sample_Harmony.png"), plot = p_sample, width = 9, height = 7, dpi = 300)
...
unique_days <- sort(unique(mckellar$injury.days))

for (day in unique_days) {
  cells_day <- WhichCells(mckellar, expression = injury.days == day)
  if (length(cells_day) > 0) {
    subset_obj <- subset(mckellar, cells = cells_day)
    
    p_day <- DimPlot(
      subset_obj,
      reduction = "umap_harmony_clean",  # Corrected embedding
      group.by = "harmony_factorIDs",
      label = TRUE,
      label.size = 3.5,
      pt.size = 0.3,
      ...
      ggtitle(paste0("McKellar – Day ", day, " (Harmony-Corrected UMAP)")) +  
      theme_minimal() +
      theme(
        plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        legend.title = element_blank(),
    ...
    fname <- file.path(umap_dir, paste0("McKellar_UMAP_day_", day, "_Harmony.png"))  
...
ggsave(
  filename = file.path(umap_dir, "McKellar_UMAP_combined_by_day_Harmony.png"),
  plot = combined_umap,
  width = 12,
  height = 10,
  dpi = 300
...
cluster_annotation_map <- mckellar@meta.data %>%
  group_by(seurat_clusters) %>%
  count(harmony_factorIDs) %>%
  slice_max(order_by = n, n = 1, with_ties = FALSE) %>%
  select(seurat_clusters, harmony_factorIDs) %>%
  arrange(as.numeric(as.character(seurat_clusters)))  # ensures sorting 

# == Save to CSV
write.csv(cluster_annotation_map,
          file.path(prop_dir, "McKellar_cluster_to_annotation_Cosgrove.csv"),
          row.names = FALSE)
...
write.csv(overall_props, file.path(prop_dir, "McKellar_overall_proportions_Cosgrove.csv"), row.names = FALSE)
saveRDS(overall_props, file.path(prop_dir, "McKellar_overall_proportions_Cosgrove_raw.rds"))
...
p_overall <- ggplot(overall_props, aes(x = harmony_factorIDs, y = Proportion, fill = harmony_factorIDs)) +
  ...
  theme_minimal() +
  labs(
    title = "McKellar – Overall Cell Type Proportions (Cosgrove Annotation)",
...
ggsave(file.path(barplot_dir, "McKellar_barplot_overall_Cosgrove.png"), plot = p_overall, width = 10, height = 6, dpi = 300)
cat(green$bold("Saved overall barplot (Cosgrove annotation).\n"))

# ─────────────────────────────────────────────────────────────────────────────
# PER SAMPLE barplots – Original 
# ─────────────────────────────────────────────────────────────────────────────
...
write.csv(sample_props, file.path(prop_dir, "McKellar_per_sample_proportions_Cosgrove.csv"), row.names = FALSE)
...
for (s in samples) {
  ...
  p <- ggplot(sample_df, aes(x = harmony_factorIDs, y = Proportion, fill = harmony_factorIDs)) +
    ...
    theme_minimal() +
    labs(
      title = paste("McKellar –", s, "(Cosgrove Annotation)"),
      x = "Cell Type", y = "Proportion"
    ) +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1, size = 9),
  ...
  ggsave(file.path(barplot_dir, paste0("McKellar_barplot_sample_", s, "_Cosgrove.png")), plot = p, width = 10, height = 6, dpi = 300)
}
cat(green$bold("Saved barplots for each sample (Cosgrove annotation).\n"))

# ─────────────────────────────────────────────────────────────────────────────
# PER DAY barplots – Original 
# ─────────────────────────────────────────────────────────────────────────────
...
write.csv(day_props, file.path(prop_dir, "McKellar_per_day_proportions_Cosgrove.csv"), row.names = FALSE)
...
p_combined <- ggplot(day_props, aes(x = harmony_factorIDs, y = Proportion, fill = harmony_factorIDs)) +
  ...
  labs(title = "McKellar – Cell Type Proportions by Day (Cosgrove)", x = "Cell Type", y = "Proportion") +
  theme(
...
ggsave(file.path(barplot_dir, "McKellar_barplot_by_day_combined_Cosgrove.png"), plot = p_combined, width = 12, height = 8, dpi = 300)
...
for (d in unique(day_props$injury.days)) {
  ...
  p <- ggplot(day_df, aes(x = harmony_factorIDs, y = Proportion, fill = harmony_factorIDs)) +
    ...
    scale_fill_manual(values = color_map) +
    theme_minimal() +
    labs(title = paste("McKellar – Day", d, "(Cosgrove Annotation)"), x = "Cell Type", y = "Proportion") +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1, size = 9),
      legend.position = "none",
      plot.title = element_text(hjust = 0.5, face = "bold")
  ...
  ggsave(file.path(barplot_dir, paste0("McKellar_barplot_day_", d, "_Cosgrove.png")), plot = p, width = 10, height = 6, dpi = 300)
}

cat(green$bold("Sved barplots for each day (Cosgrove annotation).\n"))

######################################################### SETTING UP THE MANIUAL ANNOTATION PIPELINE #########################################################

# ─────────────────────────────────────────────────────────────────────────────
# DEFINE Renad_markers BEFORE RUNNING
# ─────────────────────────────────────────────────────────────────────────────
...
marker_output     <- check_marker_coverage(Renad_markers_titlecase, mckellar, "McKellar – RNA assay")
...
generate_combined_plots(seurat_obj = mckellar,marker_list = Renad_markers_titlecase,plots_dir = renad_plot_dir)

# Only for "Dendritic_Own"
generate_combined_plots(seurat_obj = mckellar,marker_list = Renad_markers_titlecase,plots_dir = renad_plot_dir,target_cell_type = "Dendritic_own3")
...
names(top_marker_list) <- paste0("Cluster_", names(top_marker_list))  # Add prefix for clarity
